...
class NotificationManager:
    ...
    def __init__(self) -> None:
        ...
        if not all([SMTP_SERVER, SMTP_PORT, TEST_MAIL, EMAIL_PASSWORD]):
            raise ValueError("Email configuration is missing in environment variables.")
        self.smtp_server = SMTP_SERVER
        self.smtp_port = SMTP_PORT
        self.from_email = TEST_MAIL
    ...
    def send_emails(self, email_list: List[str], email_body: str) -> None:
        ...
        try:
            with smtplib.SMTP(self.smtp_server, port=self.smtp_port) as connection:
                connection.starttls()
                connection.login(user=TEST_MAIL, password=self.email_password)
