...
CHROME_PROFILE_DIR: Final[Path] = REPO_ROOT / "chrome_profile"

BASE_URL: Final[str] = "https://appbrewery.github.io/Zillow-Clone/"
...
PRICE_RE: Final[re.Pattern[str]] = re.compile(r"\$\s*\d[\d,]*")
ADDR_NOISE_RE: Final[re.Pattern[str]] = re.compile(r"[\n\r]+|[|]")
WS_RE: Final[re.Pattern[str]] = re.compile(r"\s{2,}")
...
def scrape_zillow_clone(url: str = BASE_URL) -> tuple[list[str], list[str], list[str]]:
    ...
    prices: list[str] = list(dict.fromkeys(prices_raw))
...
def compose_listings(links: list[str], prices: list[str], addresses: list[str]) -> list[Listing]:
    """抽出した値を Listing のリストにまとめる。"""
    if not links or not prices or not addresses:
        return []

    max_count = max(len(links), len(prices), len(addresses))
    count = min(len(links), len(prices), len(addresses))
    if count < max_count:
        print(
            f"[WARN] Detected mismatched counts (links={len(links)}, prices={len(prices)}, "
            f"addresses={len(addresses)}). Truncating to {count}."
        )
...
def locate_question_inputs(driver: ChromeWebDriver, timeout: int = 20) -> list[WebElement]:
    """フォーム内の入力欄（input/textarea）を順序付きで取得する。"""
    wait: WebDriverWait = WebDriverWait(driver, timeout)
    list_items: list[WebElement] = wait.until(
        EC.presence_of_all_elements_located((By.CSS_SELECTOR, "form div[role='listitem']"))
    )
    fields: list[WebElement] = []
    for item in list_items:
        field: WebElement | None = None
        for selector in ("input:not([type='hidden'])", "textarea"):
            candidates: list[WebElement] = item.find_elements(By.CSS_SELECTOR, selector)
    ...
    fallback: list[WebElement] = wait.until(
...
def fill_form_page(driver: ChromeWebDriver, listing: Listing, timeout: int = 20) -> None:
    """単一物件の入力欄に値を流し込み、送信ボタンを押す。"""
    wait: WebDriverWait = WebDriverWait(driver, timeout)
    ...
    for field_name, element in zip(FIELD_ORDER, inputs, strict=False):
    ...
    try:
        ...
        buttons: list[WebElement] = driver.find_elements(By.CSS_SELECTOR, "form div[role='button']")
        if not buttons:
            raise
...
def submit_listings_to_form(
    driver: ChromeWebDriver, form_url: str, listings: list[Listing]
) -> str | None:
    """Listing を Google フォームに連続送信し、実際に利用した URL を返す。"""
    if not listings:
...
def create_sheet_from_responses(driver: ChromeWebDriver, form_url: str) -> None:
    """回答タブを開き、必要に応じてスプレッドシートを作成する。"""
    ...
    handles_before: list[str] = list(driver.window_handles)
    sheet_button: WebElement | None = None
    ...
    for text in SHEET_CONFIRM_TEXTS:
        try:
            create_button: WebElement = dialog.find_element(By.XPATH, f".//span[contains(text(), '{text}')]")
